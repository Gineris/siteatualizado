...
if (isset($_SESSION['id_trabalhador'])) {
    ...
    // Preparar a consulta SQL para obter os dados do trabalhador
    ...
    // Contar o total de curtidas
    $sql_curtidas = "SELECT COUNT(*) AS total_curtidas FROM curtidas WHERE id_trabalhador = ?";
    $stmt_curtidas = $conn->prepare($sql_curtidas);
    if ($stmt_curtidas === false) {
        die("Erro ao preparar a consulta de curtidas: " . $conn->error);
    }

    // Vincular o parâmetro
    $stmt_curtidas->bind_param("i", $idTrabalhador);
    $stmt_curtidas->execute();
    $resultado_curtidas = $stmt_curtidas->get_result();
    $curtidas_row = $resultado_curtidas->fetch_assoc();
    $total_curtidas = $curtidas_row['total_curtidas'];

    // Fechar o statement de curtidas
    $stmt_curtidas->close();
} else {
        ...
        <div class="FotoFundo">
            ...
            <div class="BlocoPerfilPrincipal">
                ...
                <div class="LocaleLikes">
                    ...
                    <p><?php echo $total_curtidas; ?> likes</p> <!-- Mostrar total de curtidas aqui -->
                </div>
            </div>
